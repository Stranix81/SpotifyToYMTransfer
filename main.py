...
async def transfer_from_spotify_to_ym(spotify_token, ym_token): 
...
def main():
    ...
    # getting tokens
    spotify_tokens = [get_token_spotify() for _ in range(num_accounts)]
    ym_tokens = [get_token_ym() for _ in range(num_accounts)]
...
async def transfer_playlists(spotify_tokens, ym_tokens):
    ...
    for spotify_token, ym_token in zip(spotify_tokens, ym_tokens):
        # creating task
        task = asyncio.create_task(transfer_from_spotify_to_ym(spotify_token, ym_token))
